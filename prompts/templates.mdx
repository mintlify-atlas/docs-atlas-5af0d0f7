...
description: "NorthStar prompt templates support both Jinja2-style double-brace syntax and Python str.format-style single-brace variables. Missing variables raise a validation error at compile time."
---
...
with northstar.pull_prompt("summarizer", label="production") as prompt:
    with northstar.model_call("summarise", model="gpt-4o") as llm:
        with prompt.bind(
            ...
            llm.record_input_messages([{"role": "user", "content": compiled.content}])
```
...
```python
with northstar.pull_prompt("greeting") as prompt:
    with northstar.model_call("chat", model="gpt-4o-mini"):
...
All variables are typed as `"string"`, marked `required: True`, and have `default: None`. If the server returns a `PromptVersion` with an empty `variables` list, the SDK re-runs `variables_schema` locally via a Pydantic model validator so the schema is always populated before you call `compile`.
...
The following shows the full pull → compile → record flow, reading the model hint directly from the compiled prompt:
...
with northstar.pull_prompt("summarizer", label="production") as prompt:
    ...
    with northstar.model_call("summarise", model=model) as llm:
        with prompt.bind(
            variables={"doc": doc_text, "max_words": 120},
        ) as compiled:
            llm.record_input_messages([
